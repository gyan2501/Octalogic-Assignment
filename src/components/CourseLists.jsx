...
const CourseLists = () => {
  ...
  const [searchData, setsearchData] = useState(AllCourse);
  ...
  // console.log("All Courses", courses.isLoading);
  ...
  // Update searchData whenever AllCourse or query changes
  useEffect(() => {
    if (!query) {
      // If query is empty, display all courses
      setsearchData(AllCourse);
    } else {
      // Filter courses based on the search term
      const searchList = AllCourse.filter((el) => {
        ...
        {loading ? (
          ...
          <table className="min-w-full divide-y divide-gray-200 bg-white">
            ...
            <tbody className="bg-white divide-y divide-gray-200 text-tableRow text-left">
              {/* Map and display AllCourse data */}
              {searchData?.map((el, i) => (
                <tr key={i}>
